...
var swiper_custom_browser = {exports: {}};
...
(function (global, factory) {
  ...
  function addClass(className) {
    ...
    for (let i = 0; i < classes.length; i += 1) {
      for (let j = 0; j < this.length; j += 1) {
        if (
          typeof this$1$1[j] !== 'undefined' &&
          typeof this$1$1[j].classList !== 'undefined'
        ) {
  ...
  function transform(transform) {
    ...
    for (let i = 0; i < this.length; i += 1) {
      const elStyle = this$1$1[i].style;
    ...
    for (i = 0; i < this.length; i += 1) {
      if (typeof newChild === 'string') {
        ...
        for (j = tempDiv.childNodes.length - 1; j >= 0; j -= 1) {
          this$1$1[i].insertBefore(
            tempDiv.childNodes[j],
            this$1$1[i].childNodes[0]
          );
        }
      } else if (newChild instanceof Dom7) {
  ...
  function find(selector) {
    ...
    const foundElements = [];
    for (let i = 0; i < this.length; i += 1) {
      const found = this$1$1[i].querySelectorAll(selector);
  ...
  function updateActiveIndex(newActiveIndex) {
    ...
    const translate = swiper.rtlTranslate
      ? swiper.translate
      : -swiper.translate;
    const { slidesGrid } = swiper;
    const { snapGrid } = swiper;
    const { params } = swiper;
  ...
  function loopFix() {
    ...
    const { activeIndex } = swiper;
    const { slides } = swiper;
    const { loopedSlides } = swiper;
    const { allowSlidePrev } = swiper;
    const { allowSlideNext } = swiper;
    const { snapGrid } = swiper;
    const rtl = swiper.rtlTranslate;
    let newIndex;
  ...
  function setBreakpoint() {
    const swiper = this;
    const { activeIndex } = swiper;
    const { initialized } = swiper;
    let { loopedSlides } = swiper;
  ...
  const Swiper = (function (SwiperClass$$1) {
    function Swiper() {
      let assign;
    ...
    load: function load() {
      ...
      if (params.loadPrevNext) {
        if (
          ...
          // Next Slides
          for (
            let i$1 = activeIndex + slidesPerView;
            i$1 < maxIndex;
            i$1 += 1
          ) {
            if (slideExist(i$1)) {
...
}(swiper_custom_browser));

var Swiper = swiper_custom_browser.exports;
...
class ImageSlider extends HTMLElement {
  ...
  initializeSlider() {
    this.slides = JSON.parse(
      this.querySelector('script[type="application/json"]').textContent
    );

    const swiperHtml = this.getHtml(this.slides);
    const swiperFragment = document
      .createRange()
      .createContextualFragment(swiperHtml);
    const swiperElement = swiperFragment.querySelector('.swiper-container');
    const paginationElement =
      swiperFragment.querySelector('.swiper-pagination');

    this.captionTextElement = swiperFragment.querySelector(
      '.swiper-caption-text'
    );
    this.photographerElement = swiperFragment.querySelector(
      '.swiper-caption-photographer'
    );
    ...
    this.swiperInstance = new Swiper(swiperElement, {
      ...
      navigation: {
        nextEl: swiperFragment.querySelector('.swiper-button-next'),
        prevEl: swiperFragment.querySelector('.swiper-button-prev'),
      },
      a11y: {
    ...
    this.swiperInstance.on('slideChange', this.onSlideChange.bind(this));

    this.appendChild(swiperFragment);
    this.swiperInstance.update();
    this.setCaptionAndPhotographer.call(this, 0);
  }

  onSlideChange() {
    this.setCaptionAndPhotographer.call(this);
  }

  setCaptionAndPhotographer(index) {
    if (isNaN(index)) {
      index = this.swiperInstance.realIndex;
    }

    const activeSlide = this.slides[index];
    const { caption } = activeSlide.fields;
    const { photographer } = activeSlide.fields;

    const captionText = caption || '';
    this.captionTextElement.textContent = captionText;

    const photographerText = photographer ? `Foto: ${photographer}` : '';
    this.photographerElement.textContent = photographerText;
  }
  ...
  getHtml(slides) {
    return `
            ...
            <div class="swiper-caption">
                <span class="swiper-caption-text"></span>
                <span class="swiper-caption-photographer"></span>
            </div>
        `;
  }
}
...
if (customElements && customElements.define) {
  customElements.define('amedia-image-slider', ImageSlider);
} else {
  document.registerElement('amedia-image-slider', ImageSlider);
}
